...
describe('Account', function () {
  ...
  describe('new Account(number, holder)', function () {
    it('returns an Account object', function () {
      const savings = new Account(92, 'Grimdiana Bones');
      expect(savings).to.be.an('object').and.be.an.instanceof(Account);
    });

    it('has a number property that matches the number argument', function () {
      const checking = new Account(42, 'Jeff Jefferson');
      expect(checking).to.have.property('number').that.equals(42);

      const mattressCash = new Account(67, 'Jeff Jefferson');
      expect(mattressCash).to.have.property('number').that.equals(67);
    });

    it('has a holder property that matches the holder argument', function () {
      const savings = new Account(5, 'Speed Racer');
      expect(savings).to.have.property('holder').that.equals('Speed Racer');

      const debit = new Account(5, 'Slow Anne Steady');
      expect(debit).to.have.property('holder').that.equals('Slow Anne Steady');
    });
  ...
  describe('account.deposit(amount)', function () {
    ...
    context('when amount is a positive integer', function () {
      ...
      it('stores a deposit transaction with the right amount', function () {
        ...
        const transaction1 = account.transactions[0];
        ...
        const transaction2 = account.transactions[1];
